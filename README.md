...
또는 [릴리스](../../releases)의 DMG 에서 `Retto.app` 을 `Applications` 로 끌어다 놓는다.
직접 빌드한 앱은 Gatekeeper 를 거치지 않는다. DMG 는 애드혹 서명이라 첫 실행에서
시스템 설정 > 개인정보 보호 및 보안 에서 「그래도 열기」를 한 번 눌러야 한다.
