...
| 드래그 | 끄는 방향으로 달리고(행 1·2), 던지면 튕김 | 창만 옮긴다 |
...
- **끌 때 달리게 하지 않았다.** 아틀라스 행 1·2(`running-right`/`running-left`)는 대기 프레임보다
  크게 그려져 있고 꼬리가 셀 경계에서 잘려 있다. 그대로 쓰면 끌 때마다 레토가 확대되고 꼬리가 사라진다.
  `hatch-pet` 으로 두 행을 다시 생성하면 살릴 수 있다.
